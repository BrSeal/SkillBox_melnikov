...
public class ListController {
    ...
    public ResponseEntity moveUp(@PathVariable int id) {
		synchronized (Storage.class) {
			if (id <= 0||Storage.getSize()<2) return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(null);
			swap(id, id - 1);
		}
		return ResponseEntity.status(HttpStatus.OK).body(null);
	}
	...
	public ResponseEntity moveDown(@PathVariable int id) {
		synchronized (Storage.class) {
			if (id >= Storage.getSize() - 1) return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(null);
			swap(id, id + 1);
		}
		return ResponseEntity.status(HttpStatus.OK).body(null);
	}
